 ...
 **/
 component extends="cbwire-cli.models.BaseCommand" {
	...
	function run(){
        ...
        // Confirm prepare
        var confirmResponse = confirm( "Are you sure you want to install ColdBox dependencies to prepare #variables.settings.testHarnessDirectoryName# servers for running? [y/n]" );
        if( !confirmResponse ){
        ...
        printStyledMessage( "Changing to #variables.settings.testHarnessDirectoryName# directory" );
        print.line();
        command( 'cd' ).params( variables.settings.testHarnessDirectoryName ).run();
        command( 'install' ).run();
        print.line();

        printStyledMessage([
            "✅️ ColdBox CBWIRE Root dependencies installed successfully!",
            "You can now run the #variables.settings.testHarnessDirectoryName# servers",
